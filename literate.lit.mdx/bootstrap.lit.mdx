...
**The cost:** IFD breaks `--pure-eval` flag and is slower than direct imports because nix must build the tangle derivation before continuing evaluation. For a library meant to be used locally and in CI, this is acceptable. Hydra users who need pure-eval can tangle manually and point nix at the already-tangled tree.
