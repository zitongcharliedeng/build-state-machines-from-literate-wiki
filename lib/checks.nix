    ...
    for name in files:
        ...
        for i, line in enumerate(lines):
            ...
            if trimmed.startswith(fence) and not in_block:
                ...
                # Non-tangleable languages — prose illustrations, diagrams
                prose_langs = {"text", "ascii", "plaintext", "mermaid", "markdown", "md", "diff", "console", "log", "output", "example"}
                is_prose_lang = any(trimmed.startswith(fence + lang) or trimmed.startswith(fence + "{." + lang) for lang in prose_langs)
                ...
                if not has_annotation and trimmed == fence:
                    ...
                    violations += 1
                elif not has_annotation and trimmed != fence and not is_prose_lang:
