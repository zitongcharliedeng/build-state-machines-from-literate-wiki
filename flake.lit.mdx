...
## The rm verb — wikilink-aware filesystem delete

`mv` rewrites links to follow a moved target; `rm` does the dual: it deletes a file and replaces every inbound wikilink with a `[[REMOVED: <old-target>]]` marker. The deletion site becomes visibly broken in prose rather than silently dangling — readers see the gap, automated tooling can grep for `REMOVED:` to surface obsolete references, and the original target name is preserved for forensic purposes (log, recover, decide whether the reference should be re-targeted).

Same shape as `mv`: tracked sources go through `git rm`, untracked through plain `rm`. The walker uses the same find-with-submodule-prune logic and the same regex over the four wikilink shapes (bare, path, anchor, anchor-and-alias). The only difference is the replacement target — instead of substituting a new path, it substitutes the visible REMOVED marker that preserves the alias text when one was present.

```{.nix file=lib/init.nix as-a-real-non-nix-store-file="init module imported by the bootstrap"}
      rmVerb = pkgs.writeShellApplication {
        name = "lsmw-rm";
        runtimeInputs = with pkgs; [ git findutils gnused coreutils ];
        text = ''
          set -euo pipefail
          if [ "$#" -ne 1 ]; then
            echo "usage: lsmw rm <path>" >&2
            exit 2
          fi
          src="$1"
          if [ ! -e "$src" ]; then
            echo "lsmw rm: source does not exist: $src" >&2
            exit 1
          fi
          repo_root="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
          src_abs="$(realpath -- "$src")"
          rel_old="$(realpath --relative-to="$repo_root" -- "$src_abs" 2>/dev/null || echo "$src")"
          strip_md_ext() {
            local p="$1"
            p="''${p%.lit.mdx}"
            p="''${p%.md}"
          ...
          while IFS= read -r -d "" f; do
            base_old_count=$((base_old_count + 1))
              ...
              \( -name .git -prune \) -o \
              \( -type d ! -path "$repo_root" -exec test -e {}/.git \; -prune \) -o \
              \( -type f \( -name "$base_old.md" -o -name "$base_old.mdx" -o -name "$base_old.lit.mdx" \) -print0 \))
          if git ls-files --error-unmatch -- "$src" >/dev/null 2>&1; then
          ...
          path_old_re="$(esc "$path_old")"
          base_old_re="$(esc "$base_old")"
          path_old_lit="$(printf '%s' "$path_old" | sed -e 's/[\&/]/\\&/g')"
          base_old_lit="$(printf '%s' "$base_old" | sed -e 's/[\&/]/\\&/g')"
          tmp_list="$(mktemp)"
          trap 'rm -f "$tmp_list"' EXIT
          find "$repo_root" \
              \( -name .git -prune \) -o \
          ...
          while IFS= read -r -d "" f; do
            sed -i \
              -e "s/\[\[$path_old_re\(#[^]|]*\)\?\(|[^]]*\)\?\]\]/[[REMOVED: $path_old_lit]]/g" \
              "$f"
            if [ "$base_old_count" -le 1 ] && [ "$base_old" != "$path_old" ]; then
              sed -i \
                -e "s/\[\[$base_old_re\(#[^]|]*\)\?\(|[^]]*\)\?\]\]/[[REMOVED: $base_old_lit]]/g" \
                "$f"
            fi
          done < "$tmp_list"
          if [ "$base_old_count" -gt 1 ] && [ "$base_old" != "$path_old" ]; then
            echo "lsmw rm: bare basename '$base_old' is not unique ($base_old_count files); bare-basename links left untouched" >&2
          fi
          echo "lsmw rm: $rel_old (links replaced with [[REMOVED: ...]])"
        '';
      };
```

## The todo verb — aggregate TaskNotes (inline + standalone)

LSMW projects integrate with [Obsidian TaskNotes](https://tasknotes.dev) in two formats:

- **Inline (preferred)** — a checklist line in literate prose, tagged with `#tasknotes`, optionally linking to a standalone TaskNote: `- [ ] Open upstream PR ... #tasknotes #forgejo [[task-pr-upstream]]`. The inline line is what a reader sees in context; the wikilink resolves to the standalone file holding metadata (due dates, priority, project links).
- **Standalone** — a separate `.md` file whose frontmatter declares `tags: [task]` per the TaskNotes plugin spec. The body is the task description; frontmatter holds `due`, `priority`, `status`, etc.

`lsmw todo` walks the tree once, finds both forms, and prints them as `file:line | kind | status | description` sorted by file:line. No external API call (TaskNotes is an Obsidian plugin running client-side on the user's vault, not a service); the literate prose IS the task DB and `lsmw todo` is a CLI mirror of it. The walker uses the same `.git` and submodule prune logic as `mv`/`rm`. Only `.md` / `.mdx` / `.lit.mdx` files are scanned. Inline match: `^- \[([ x])\] .*#tasknotes`. Standalone match: a frontmatter `tags:` block whose array contains `task` (covers both `tags: [task]` flow and `tags:\n  - task` block-list forms). Checked status (`[x]`) is preserved so `lsmw todo | grep '| done'` audits completed work.

```{.nix file=lib/init.nix as-a-real-non-nix-store-file="init module imported by the bootstrap"}
      todoVerb = pkgs.writeShellApplication {
        name = "lsmw-todo";
        runtimeInputs = with pkgs; [ git findutils gnugrep gawk coreutils ];
        text = ''
          set -euo pipefail
          repo_root="$(git rev-parse --show-toplevel 2>/dev/null || pwd)"
          tmp_list="$(mktemp)"
              ...
              > "$tmp_list"
          {
            while IFS= read -r -d "" f; do
              rel="''${f#"$repo_root"/}"
              # Inline: checklist lines tagged #tasknotes
              (grep -nE '^- \[[ xX]\] .*#tasknotes' "$f" 2>/dev/null || true) | while IFS=: read -r ln body; do
                [ -n "''${ln:-}" ] || continue
                status="todo"
                if printf '%s' "$body" | grep -qE '^- \[[xX]\]'; then
                  status="done"
                fi
                desc=$(printf '%s' "$body" | sed -E 's/^- \[[ xX]\] //')
                printf '%s:%s | inline | %s | %s\n' "$rel" "$ln" "$status" "$desc"
              done
              # Standalone: frontmatter `tags:` containing `task`. Two YAML
              # shapes covered: flow `tags: [task, ...]` and block list with
              # `- task` under `tags:`. Frontmatter terminator is `^---$`.
              awk -v rel="$rel" '
                NR==1 && /^---$/ { in_fm=1; next }
                in_fm && /^---$/ { exit }
                in_fm && /^tags:[[:space:]]*\[/ {
                ...
                in_tags && /^[[:space:]]*-[[:space:]]+task[[:space:]]*$/ { is_task=1 }
                in_fm && /^status:[[:space:]]*/ { sub(/^status:[[:space:]]*/, ""); st=$0 }
                END {
                  if (is_task) {
                    if (st == "") st = "todo"
                    printf "%s:1 | standalone | %s | %s\n", rel, st, rel
                  }
                }
              ' "$f" 2>/dev/null || true
            done < "$tmp_list"
      ...
      cli = pkgs.writeShellScriptBin "literate-state-machine-wiki" ''
        ...
        case "''${1:-}" in
          ...
          *)
            ...
            echo "  literate-state-machine-wiki todo                 # list #tasknotes-tagged checklist items"
            exit 1
            ;;
        esac
      '';
...
```{.nix file=lib/init.nix as-a-real-non-nix-store-file="init module imported by the bootstrap"}
  mkChecks = { pkgs, tangled, pipeline, checksLib, init, src }:
    let
      ...
      probeRm = probeInit.packages.${pkgs.stdenv.hostPlatform.system}.rmVerb;
      rmInvalidatesWikilinks = pkgs.runCommand "rm-invalidates-wikilinks" {
        nativeBuildInputs = [ pkgs.git probeRm ];
      } ''
        export HOME=$TMPDIR
        ...
        printf '%s\n' \
          ...
          'Both: [[gone#sec|click]].' \
          'Path: [[subdir/gone]].' \
          'PathAlias: [[subdir/gone|c]].' \
          'Unrelated: [[other]].' \
          > ref.md
        echo "x" > subdir/gone.md
        echo "y" > other.md
        git init -q
        git add -A
        git commit -qm init
        lsmw-rm subdir/gone.md
        if [ -e subdir/gone.md ]; then
          echo "FAIL: subdir/gone.md was not removed" >&2
          exit 1
        fi
        if grep -q '\[\[gone' ref.md; then
          echo "FAIL: bare/anchor/alias 'gone' references not invalidated" >&2
          cat ref.md >&2
          exit 1
        fi
        if grep -q '\[\[subdir/gone' ref.md; then
          echo "FAIL: path 'subdir/gone' references not invalidated" >&2
          cat ref.md >&2
          exit 1
        fi
        if ! grep -q '\[\[REMOVED: subdir/gone\]\]' ref.md; then
          echo "FAIL: expected [[REMOVED: subdir/gone]] marker present" >&2
          cat ref.md >&2
          exit 1
        fi
        if ! grep -q '\[\[REMOVED: gone\]\]' ref.md; then
          echo "FAIL: expected [[REMOVED: gone]] marker for bare-basename links" >&2
          cat ref.md >&2
          exit 1
        fi
        if ! grep -qF '[[other]]' ref.md; then
          echo "FAIL: unrelated [[other]] reference was incorrectly modified" >&2
          cat ref.md >&2
          exit 1
        fi
        touch "$out"
    ...
    in {
      ...
      rm-invalidates-wikilinks = rmInvalidatesWikilinks;
    }
