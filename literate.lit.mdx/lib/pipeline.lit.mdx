...
## tanglePerFile
`tanglePerFile` fans out `tangleOneFile` across every literate source found by `enumerateLiterateFiles`, then merges the per-file `$out` trees into one store path via `pkgs.symlinkJoin`. The merged output is byte-identical to what `tangle` produces, but edits to one `.lit.mdx` now invalidate exactly one `runCommand` instead of re-tangling all 79 files. The `passthru.perFile` attribute exposes the individual per-file derivations for downstream tools that want to consume just one slice.
```{.nix file=lib/pipeline.nix as-a-real-non-nix-store-file="flake.nix imports this module"}
  tanglePerFile = {
    src,
    sourceDir,
