...
**Inline tasknotes are bidirectional primitives.** A bullet `- [[Do this daily]]` in any vault file points to the standalone TaskNote `Do this daily.md` — but absent extra metadata, an agent reading the task file cannot tell where it was inlined without scanning the whole vault (O(N) per task). That's a real gap in upstream TaskNotes. `lsmw todo inline <file> '<title>'` closes it by maintaining the link from BOTH sides under one flock: it appends `- [[<title>]]` to `<file>` AND appends `<file>` to the task file's `referenced_in:` frontmatter list (created if absent, deduplicated if already present). An agent reading the task file thereafter sees its inline-reference sites in O(1) without grepping. Origin tracking is a primitive of the lsmw API, not a search.
            ...
            inline)
              ...
              (
                ...
                yq -i --front-matter=process \
                  ".referenced_in = ((.referenced_in // []) + [\"$rel\"] | unique)" \
                  "$task_file"
              ) 200>"$vault/.lsmw.lock"
              ;;
            *)
