...
## The mv and rm verbs — thin wrappers around obsidian-cli

`obsidian-cli` (Yakitrak's [notesmd-cli](https://github.com/Yakitrak/notesmd-cli), headless, no Obsidian process required) is the UNIX library that already does wikilink-aware rename and delete "one thing well." `lsmw mv` and `lsmw rm` exec it verbatim. No bespoke link-rewrite, no `find`-with-prune walker, no sed regex over four wikilink shapes — that work belongs in obsidian-cli where it's tested by its own community. obsidian-cli is a Go binary not yet in nixpkgs; consumers install it on PATH (`go install github.com/Yakitrak/notesmd-cli@latest`) and the wrappers exec it. If it's missing, the wrappers fail with a clear hint.

```{.nix file=lib/init.nix as-a-real-non-nix-store-file="init module imported by the bootstrap"}
      mvVerb = pkgs.writeShellApplication {
        name = "lsmw-mv";
        text = ''
          command -v obsidian-cli >/dev/null || { echo "lsmw mv requires obsidian-cli on PATH (go install github.com/Yakitrak/notesmd-cli@latest)" >&2; exit 1; }
          exec obsidian-cli move "$@"
        '';
      };
      rmVerb = pkgs.writeShellApplication {
        name = "lsmw-rm";
        text = ''
          command -v obsidian-cli >/dev/null || { echo "lsmw rm requires obsidian-cli on PATH (go install github.com/Yakitrak/notesmd-cli@latest)" >&2; exit 1; }
          exec obsidian-cli delete "$@"
        '';
      };
```
