...
TOML
      ...
      # Filter postTangle by --until (if set, only run hooks up to and including the named one)
      effectivePostTangle = if until == null then postTangle else
        let
          takeUntil = name: hooks:
            let go = acc: remaining:
              if remaining == [] then acc
              else let h = builtins.head remaining; rest = builtins.tail remaining;
              in if h.name == name then acc ++ [h]
              else go (acc ++ [h]) rest;
            in go [] hooks;
        in takeUntil until postTangle;
      ...
      postTangled = if effectivePostTangle == [] then tangledTree else
